...
try {
  ...
  await prisma.$transaction(async tx => {
    const [lock] = await tx.$queryRaw`SELECT pg_try_advisory_xact_lock(hashtext('ridesafe:first-admin')) AS acquired`
    if (!lock?.acquired) throw new Error('Another Super Admin bootstrap is currently running. Try again shortly.')
