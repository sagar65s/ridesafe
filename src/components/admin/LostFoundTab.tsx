    ...
    <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }}>
      ...
      <div className="glass-panel" style={{ padding:'2rem' }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'1.5rem', flexWrap:'wrap', gap:'1rem' }}>
          <div>
            <h3 style={{ margin:0, fontSize:'1.3rem' }}><TranslatedText text={"Lost & Found"}/></h3>
            <div style={{ fontSize:'0.85rem', color:'var(--text-muted)', marginTop:4 }}>{items.filter(i => i.status === 'OPEN').length}<TranslatedText text={" open reports"}/></div>
          </div>
          <motion.button whileHover={{ scale:1.04 }} whileTap={{ scale:0.96 }} className="btn btn-primary" onClick={() => setShowModal(true)}><TranslatedText text={"+ Report Item"}/></motion.button>
        </div>
            ...
            <motion.div key={item.id} initial={{ opacity:0 }} animate={{ opacity:1 }}
              ...
              <div style={{ display:'flex', alignItems:'center', gap:'1rem' }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize:'0.8rem', color:'var(--text-muted)' }}><TranslatedText text={" Reported by "}/>{item.reporter.name} · {new Date(item.createdAt).toLocaleDateString()}
                  </div>
                </div>
              </div>
              <div style={{ display:'flex', gap:'0.5rem', alignItems:'center', flexWrap:'wrap' }}>
                <span className="badge" style={{ background:`${STATUS_COLOR[item.status]}22`, color:STATUS_COLOR[item.status] }}><TranslatedText text={item.status}/></span>
                {item.status === 'OPEN' && (
                  <motion.button whileTap={{ scale:0.9 }} className="btn" onClick={() => updateStatus(item.id, 'FOUND')}
                    style={{ padding:'0.4rem 0.7rem', fontSize:'0.75rem', background:'rgba(16,185,129,0.1)', color:'var(--success)' }}><TranslatedText text={" Mark Found "}/></motion.button>
                )}
                {item.status === 'FOUND' && (
                  <motion.button whileTap={{ scale:0.9 }} className="btn" onClick={() => updateStatus(item.id, 'CLOSED')}
                    style={{ padding:'0.4rem 0.7rem', fontSize:'0.75rem', background:'rgba(107,114,128,0.1)', color:'#6B7280' }}><TranslatedText text={" Close "}/></motion.button>
                )}
              </div>
            </motion.div>
          ))}
          {items.length === 0 && <div style={{ display:'flex', flexDirection:'column', alignItems:'center', padding:'3rem', color:'var(--text-muted)' }}><Sparkles size={32} style={{marginBottom: '1rem'}}/><TranslatedText text={"No reports yet — that's great!"}/></div>}
        </div>
      </div>
        ...
        {showModal && (
          <motion.div className="modal-overlay" initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }} onClick={e => { if (e.target === e.currentTarget) setShowModal(false) }}>
            <motion.div className="modal-box" initial={{ scale:0.9 }} animate={{ scale:1 }} exit={{ scale:0.9 }}>
              <h3 style={{ marginBottom:'1.5rem' }}><TranslatedText text={"Report Lost Item"}/></h3>
              <textarea className="input-field" rows={4} placeholder={translateUi("Describe the lost item (e.g., blue water bottle, left on Bus WKA1234)...")} value={description} onChange={e => setDescription(e.target.value)} style={{ resize:'vertical', minHeight:100 }} />
              <div style={{ display:'flex', gap:'1rem', marginTop:'1.5rem' }}>
                ...
                <motion.button whileTap={{ scale:0.97 }} className="btn btn-primary" style={{ flex:2 }} onClick={handleReport}><TranslatedText text={"Submit Report"}/></motion.button>
              </div>
            </motion.div>
          </motion.div>
        )}
