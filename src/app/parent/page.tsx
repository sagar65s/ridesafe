...
interface StudentData {
  ...
  bus?: { busNumber?:string; plateNumber:string; driver?:{name:string;phone?:string;personnelType?:string} };
  route?: { name: string; buses?: { busNumber?:string; plateNumber:string; driver?:{name:string;phone?:string;personnelType?:string} }[] };
}
...
interface PaymentData { id:string; bukkuInvoiceId?:string; amount:number; status:string; dueDate?:string; createdAt:string }
interface IssueData { id:string; category:string; subject:string; description:string; priority:string; status:string; resolution?:string; createdAt:string }
interface TripHistoryData {
  id:string; date:string; status:string; routeName:string; driverName:string; busNumber?:string|null; delayMinutes:number; delayReason?:string|null;
  pickedUp:number; droppedOff:number; absent:number;
  attendance:{studentId:string;studentName:string;action:string;timestamp:string}[];
}
...
import { Target, Flame, ShieldCheck, Sunrise, AlertTriangle, CheckCircle, AlertCircle, Trophy, Medal, Award, Phone, Bus, Clock, Clipboard, Home, XCircle, User, Settings, Bell, HelpCircle, LogOut, Globe, MapPin, History } from 'lucide-react'
...
export default function ParentDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'HOME' | 'MY_CHILD' | 'HISTORY' | 'MESSAGES' | 'PROFILE'>('HOME')
  const [showMap, setShowMap] = useState(false)
  const [busNearby, setBusNearby] = useState(false)
  const [nearbyMessage, setNearbyMessage] = useState('Your bus is approaching! Get ready now.')
  const nearbyTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null)
  const previousEtaRef = useRef<number | null>(null)
  ...
  const [issueForm,setIssueForm]=useState({studentId:'',category:'DELAY',subject:'',description:'',priority:'NORMAL'})
  const [submittingIssue,setSubmittingIssue]=useState(false)
  const [tripHistory,setTripHistory]=useState<TripHistoryData[]>([])

  // Profile menu state
  const [me, setMe] = useState<{ id:string; name: string; email: string; phone?: string } | null>(null)
  const [profilePanel, setProfilePanel] = useState<'INFO' | 'HELP' | 'SETTINGS' | 'NOTIFICATIONS' | null>(null)
  ...
  useEffect(()=>{ fetch('/api/issues').then(r=>r.ok?r.json():{issues:[]}).then(d=>setIssues(d.issues||[])).catch(()=>{}) },[])
  useEffect(()=>{ fetch('/api/trips/history').then(r=>r.ok?r.json():{trips:[]}).then(d=>setTripHistory(d.trips||[])).catch(()=>{}) },[])

  const submitIssue=async()=>{
    if(!issueForm.subject.trim()||issueForm.description.trim().length<10){setMsgToast('Please enter a subject and at least 10 characters of detail.');return}
    setSubmittingIssue(true)
    const response=await fetch('/api/issues',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(issueForm)})
    const result=await response.json().catch(()=>({}))
    if(response.ok){setMsgToast('Transport issue submitted to the school.');setIssueForm({studentId:'',category:'DELAY',subject:'',description:'',priority:'NORMAL'});const refreshed=await fetch('/api/issues').then(r=>r.json());setIssues(refreshed.issues||[])}
    else setMsgToast(result.error||'Unable to submit issue')
    setSubmittingIssue(false);setTimeout(()=>setMsgToast(''),3500)
  }
  ...
  // Proximity is based on this parent's linked child's stop, not simulated
  // movement. Alert once as the ETA crosses two minutes and one minute.
  const checkBusProximity = (driversData: DriverData[], studentsData: StudentData[]) => {
    ...
    if (activeDriver?.etaMins == null) return
    const previousEta = previousEtaRef.current
    const threshold = activeDriver.etaMins <= 1 && (previousEta == null || previousEta > 1)
      ? 1
      : activeDriver.etaMins <= 2 && (previousEta == null || previousEta > 2) ? 2 : null
    if (threshold !== null) {
      const message = `Bus arriving in about ${threshold} minute${threshold === 1 ? '' : 's'}${activeDriver.targetStop ? ` at ${activeDriver.targetStop}` : ''}. Please be ready.`
      setNearbyMessage(message)
      setBusNearby(true)
      playHorn()
      if (typeof Notification !== 'undefined' && Notification.permission === 'granted') {
        new Notification('RideSafe bus alert', { body: message, icon: '/ridesafe-mark.svg' })
      }
      if (nearbyTimerRef.current) clearTimeout(nearbyTimerRef.current)
      nearbyTimerRef.current = setTimeout(() => setBusNearby(false), 20000)
    }
    previousEtaRef.current = activeDriver.etaMins
  }
  ...
  useEffect(() => {
    ...
    const fetchInitialData = async () => {
      try {
        ...
        if (meRes.ok) {
          ...
          if (meData.user) {
            setMe(meData.user)
            fetch(`/api/billing/${meData.user.id}`).then(r => r.ok ? r.json() : { payments:[] }).then(d => setPayments(d.payments || [])).catch(() => {})
          }
        }

        if (meRes.status === 401 || studentsRes.status === 401) { router.push('/'); return }
    ...
    let refreshingLocation = false
    let lastLocationRefresh = 0
    const refreshLocation = async () => {
      if (refreshingLocation || Date.now() - lastLocationRefresh < 5000) return
      refreshingLocation = true
      lastLocationRefresh = Date.now()
      try {
        const response = await fetch('/api/location')
        if (response.ok) setDrivers((await response.json()).drivers || [])
      } catch (error) { console.error(error) }
      finally { refreshingLocation = false }
    }
    // Polling also updates hardware GPS and recovers from disconnected SSE.
        ...
        if (newNotifs.length > 0) {
          ...
          setLastNotifId(prev => {
            if (prev !== null && prev !== latest.id) {
              ...
              else if (latest.type === 'BUS_ETA_2_MIN' || latest.type === 'BUS_ETA_1_MIN') playHorn()
              if (typeof Notification !== 'undefined' && Notification.permission === 'granted') {
                try {
                  navigator.serviceWorker?.ready.then(reg => {
                    reg.showNotification(latest.title, { body: latest.body, icon: '/ridesafe-mark.svg' })
    ...
    evtSource.onmessage = () => { void refreshLocation() }
    evtSource.onerror = () => { void refreshLocation() }
  ...
  // ── Tab icons ───────────────────────────────────────────────────────────────
  const NAV_TABS = [
    { key:'HOME',       label:t('parent.home'),     icon:(c:string)=><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="2"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg> },
    { key:'MY_CHILD',  label:t('parent.myChild'),  icon:(c:string)=><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg> },
    { key:'HISTORY', label:t('nav.history'), icon:(c:string)=><History size={22} color={c}/> },
    ...
    { key:'PROFILE',   label:t('parent.profile'),   icon:(c:string)=><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg> },
  ] as const
      ...
      <div style={{ height:'100vh', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', background:'var(--bg-color)' }}>
         <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} style={{ textAlign: 'center' }}>
            ...
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
               ...
               {(!isAlertReady || !isHornReady) ? t('common.loading') : t('parent.initializeTracker')}
            </motion.button>
         </motion.div>
      </div>
    )
        ...
        <div className="mobile-header">
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
            <div style={{ display:'flex', alignItems:'center', gap:10 }}>
              <RideSafeLogo compact height={30} />
              <div>
        ...
        <div style={{ padding:'16px 16px 0 16px' }}>
          ...
          {activeTab === 'HOME' && (
            <motion.div initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }}>
              ...
              <h3 style={{ fontSize:'15px', margin:'20px 0 10px 0', color:'var(--text-main)' }}>{t('parent.recentActivity')}</h3>
            ...
            <motion.div initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }}>
                  ...
                  <div className="mobile-card" style={{ padding:0, overflow:'hidden', marginBottom:14 }}>
                    ...
                    <div style={{ background:'linear-gradient(135deg,#4f46e5,#6366f1)', padding:'16px', display:'flex', alignItems:'center', gap:14 }}>
                      ...
                      <div style={{ flex:1 }}>
                        ...
                        <div style={{ fontSize:13, color:'rgba(255,255,255,0.75)', marginTop:2 }}>{student.grade}{student.className?` · ${student.className}`:''}{student.section?`-${student.section}`:''} · {student.level}</div>
                    ...
                    <div style={{ padding:'12px 16px', display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px' }}>
                      {[
                        ...
                        { icon:<Bus size={14}/>, label:'Bus', val:student.isSelfPickup?'Self-Pickup':(student.bus?.busNumber||student.bus?.plateNumber||student.route?.buses?.[0]?.busNumber||student.route?.buses?.[0]?.plateNumber||'Not assigned') },
                        { icon:<Clock size={14}/>, label:'Pickup Time', val:student.pickupTime||'On-Route' },
                        { icon:<Clipboard size={14}/>, label:'Student ID', val:student.studentCode||`STU-${student.id.slice(-5).toUpperCase()}` },
                        { icon:<MapPin size={14}/>, label:'Route', val:student.route?.name||'Not assigned' },
                        { icon:<MapPin size={14}/>, label:'Pickup', val:student.pickupStop?.name||student.pickupAddress||'Not assigned' },
                        { icon:<MapPin size={14}/>, label:'Drop', val:student.dropoffStop?.name||student.dropoffAddress||'Not assigned' },
                        { icon:<Phone size={14}/>, label:'Driver / Maintainer', val:student.bus?.driver?.name||student.route?.buses?.[0]?.driver?.name||'Not assigned' },
                      ].map(({ icon, label, val }) => (
                        <div key={label}>
              ...
              {students.length === 0 && (
                <div className="mobile-card" style={{ textAlign:'center', padding:32 }}>
                  ...
                  <div style={{ fontWeight:600, marginTop:8 }}>{t('parent.noChild')}</div>
                  <div style={{ fontSize:13, color:'var(--text-muted)', marginTop:4 }}>{t('parent.noChildHelp')}</div>
                </div>
              )}
            </motion.div>
          )}
          ...
          {activeTab === 'HISTORY' && (
            <motion.div initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }}>
              <div className="mobile-card" style={{ marginBottom:14 }}>
                <h2 style={{ margin:'0 0 4px', fontSize:19 }}>Trip History</h2>
                <div style={{ color:'var(--text-muted)', fontSize:12 }}>Only trips and attendance belonging to your linked children are shown.</div>
              </div>
              <div style={{ display:'grid', gap:12 }}>
                {tripHistory.map(trip => (
                  <div key={trip.id} className="mobile-card" style={{ marginBottom:0 }}>
                    <div style={{ display:'flex', justifyContent:'space-between', gap:10, alignItems:'flex-start' }}>
                      <div><strong>{trip.routeName}</strong><div style={{ fontSize:12, color:'var(--text-muted)', marginTop:3 }}>{new Date(trip.date).toLocaleString()} · {trip.busNumber || 'Bus not recorded'}</div></div>
                      <span className={`badge ${trip.status === 'TRIP_COMPLETED' ? 'badge-success' : 'badge-info'}`}>{trip.status.replaceAll('_',' ')}</span>
                    </div>
                    <div style={{ fontSize:12, color:'var(--text-muted)', marginTop:9 }}>Driver / Maintainer: {trip.driverName}{trip.delayMinutes ? ` · Delayed ${trip.delayMinutes} min${trip.delayReason ? ` (${trip.delayReason})` : ''}` : ''}</div>
                    <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:7, marginTop:10 }}>
                      {[['Boarded',trip.pickedUp],['Dropped',trip.droppedOff],['Absent',trip.absent]].map(([label,value]) => <div key={String(label)} style={{ padding:8, textAlign:'center', borderRadius:9, background:'var(--surface-2)' }}><strong>{value}</strong><div style={{ fontSize:10, color:'var(--text-muted)' }}>{label}</div></div>)}
                    </div>
                    {trip.attendance.length > 0 && <div style={{ display:'grid', gap:6, marginTop:10 }}>{trip.attendance.map((attendance,index) => <div key={`${attendance.studentId}-${attendance.timestamp}-${index}`} style={{ display:'flex', justifyContent:'space-between', gap:8, fontSize:12, borderTop:'1px solid var(--surface-border)', paddingTop:7 }}><span>{attendance.studentName} · {attendance.action.replaceAll('_',' ')}</span><span style={{ color:'var(--text-muted)', whiteSpace:'nowrap' }}>{new Date(attendance.timestamp).toLocaleTimeString()}</span></div>)}</div>}
                  </div>
                ))}
                {tripHistory.length === 0 && <div className="mobile-card" style={{ textAlign:'center', color:'var(--text-muted)', padding:28 }}>No trip history is available yet.</div>}
              </div>
            </motion.div>
          )}
          ...
          {activeTab === 'MESSAGES' && (
            <motion.div initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }}>
              ...
              <div className="mobile-card" style={{marginBottom:14}}>
                <h3 style={{margin:'0 0 10px',fontSize:15}}>Report a Transport Issue</h3>
                <div style={{display:'grid',gap:9}}>
                  <select className="select-field" value={issueForm.studentId} onChange={e=>setIssueForm({...issueForm,studentId:e.target.value})}><option value="">General issue</option>{students.map(student=><option key={student.id} value={student.id}>{student.name}</option>)}</select>
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}><select className="select-field" value={issueForm.category} onChange={e=>setIssueForm({...issueForm,category:e.target.value})}>{['DELAY','DRIVER','BUS','ROUTE','SAFETY','ATTENDANCE','OTHER'].map(v=><option key={v} value={v}>{v}</option>)}</select><select className="select-field" value={issueForm.priority} onChange={e=>setIssueForm({...issueForm,priority:e.target.value})}>{['LOW','NORMAL','HIGH','URGENT'].map(v=><option key={v} value={v}>{v}</option>)}</select></div>
                  <input className="input-field" maxLength={120} placeholder="Issue subject" value={issueForm.subject} onChange={e=>setIssueForm({...issueForm,subject:e.target.value})}/>
                  <textarea className="input-field" rows={3} maxLength={2000} placeholder="Explain the transport issue" value={issueForm.description} onChange={e=>setIssueForm({...issueForm,description:e.target.value})}/>
                  <button className="btn btn-primary" disabled={submittingIssue} onClick={submitIssue}>{submittingIssue?'Submitting…':'Submit Issue'}</button>
                </div>
                {issues.length>0&&<div style={{display:'grid',gap:7,marginTop:12}}>{issues.slice(0,5).map(issue=><div key={issue.id} style={{padding:9,borderRadius:8,background:'var(--surface-2)',fontSize:12}}><strong>{issue.subject}</strong><span className="badge badge-info" style={{float:'right'}}>{issue.status.replaceAll('_',' ')}</span>{issue.resolution&&<div style={{color:'var(--success)',marginTop:4}}>{issue.resolution}</div>}</div>)}</div>}
              </div>
          ...
          {activeTab === 'PROFILE' && (
            <motion.div initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }}>
              <div className="mobile-card" style={{ padding:'16px', marginBottom:14 }}>
                <div style={{ fontWeight:700, fontSize:15, marginBottom:10 }}>Invoices & Payments</div>
                <div style={{ display:'grid', gap:8 }}>
                  {payments.slice(0, 6).map(payment => (
                    <div key={payment.id} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', gap:12, padding:'10px 12px', borderRadius:10, background:'var(--surface-2)', border:'1px solid var(--surface-border)' }}>
                      <div>
                        <div style={{ fontWeight:700, fontSize:14 }}>RM {Number(payment.amount).toFixed(2)}</div>
                        <div style={{ fontSize:11, color:'var(--text-muted)', marginTop:2 }}>{payment.bukkuInvoiceId || payment.id.slice(-8)}{payment.dueDate ? ` · Due ${new Date(payment.dueDate).toLocaleDateString()}` : ''}</div>
                      </div>
                      <span className={`badge ${payment.status === 'PAID' ? 'badge-success' : payment.status === 'PENDING_PROVIDER' ? 'badge-warning' : 'badge-info'}`} style={{ fontSize:10 }}>{payment.status.replaceAll('_',' ')}</span>
                    </div>
                  ))}
                  {payments.length === 0 && <div style={{ textAlign:'center', color:'var(--text-muted)', fontSize:13, padding:12 }}>No invoices yet</div>}
                </div>
              </div>
              {students.map(student => (
              ...
              <div style={{ background:'var(--surface)', border:'1px solid var(--surface-border)', borderRadius:16, overflow:'hidden', boxShadow:'0 1px 3px rgba(0,0,0,0.05)' }}>
                {[
                  ...
                  { icon:<Settings size={20}/>, label:'Settings & Preferences', onClick: () => setProfilePanel(p => p === 'SETTINGS' ? null : 'SETTINGS') },
                  { icon:<Bell size={20}/>, label:'Notification Settings', onClick: () => setProfilePanel(p => p === 'NOTIFICATIONS' ? null : 'NOTIFICATIONS') },
                ...
                <div onClick={handleLogout} style={{ display:'flex', alignItems:'center', padding:'18px 16px', cursor:'pointer', background:'rgba(239,68,68,0.12)', borderTop:'1px solid rgba(239,68,68,.25)' }}>
                  <span style={{ display:'flex', marginRight:14 }}><LogOut size={20} color="var(--danger)"/></span>
                  <div style={{ flex:1, fontSize:'15px', fontWeight:700, color:'var(--danger)' }}>{t('common.logout')}</div>
                </div>
              </div>
              ...
              {profilePanel === 'SETTINGS' && (
                <motion.div initial={{ opacity:0, height:0 }} animate={{ opacity:1, height:'auto' }} className="mobile-card" style={{ padding:'18px 16px', marginTop:10 }}>
                  <div style={{ fontSize:14, lineHeight:1.6 }}>Choose your preferred app language below. The selection is saved to this parent account and follows you on your next login.</div>
                </motion.div>
              )}
              ...
              {profilePanel === 'NOTIFICATIONS' && (
                <motion.div initial={{ opacity:0, height:0 }} animate={{ opacity:1, height:'auto' }} className="mobile-card" style={{ padding:'18px 16px', marginTop:10 }}>
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', gap:12 }}>
                    <div>
                      <div style={{ fontWeight:700 }}>Bus arrival notifications</div>
                      <div style={{ color:'var(--text-muted)', fontSize:12, marginTop:3 }}>Status: {notificationPermission}</div>
                    </div>
                    <button className="btn btn-primary" disabled={notificationPermission === 'granted' || notificationPermission === 'unsupported'} onClick={async () => {
                      if (typeof Notification === 'undefined') return
                      const permission = await Notification.requestPermission()
                      setNotificationPermission(permission)
                      if (permission === 'granted') playHorn()
                    }}>{notificationPermission === 'granted' ? 'Enabled' : 'Enable'}</button>
                  </div>
                </motion.div>
              )}
