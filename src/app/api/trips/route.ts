...
export async function GET(request: NextRequest) {
    try {
        ...
        if (user.role === 'DRIVER') {
            ...
            const assignments = students.filter(student => student.routeId).map(student => ({ routeId: student.routeId as string, ...(student.busId ? { busId: student.busId } : {}) }))
            const trips = await prisma.trip.findMany({
...
export async function POST(request: NextRequest) {
    try {
        ...
        const [driver, route, bus] = await Promise.all([
            ...
            data.busId ? prisma.bus.findUnique({ where: { id: data.busId }, select: { organizationId: true, driverId: true, routeId: true, status: true } }) : Promise.resolve(null),
        ])
        ...
        if (bus && (bus.status !== 'ACTIVE' || bus.organizationId !== route.organizationId || (bus.routeId && bus.routeId !== data.routeId) || (user.role === 'DRIVER' && bus.driverId !== user.id))) {
            return NextResponse.json({ error: 'Invalid bus assignment' }, { status: 400 })
        }
        if (user.role === 'DRIVER') {
            const assignedBus = await prisma.bus.findFirst({ where: { driverId: user.id, routeId: data.routeId, status: 'ACTIVE', ...(data.busId ? { id: data.busId } : {}) }, select: { id: true } })
        ...
        const trip = await prisma.$transaction(async tx => {
            ...
            return tx.trip.create({ data: { routeId: data.routeId, driverId, busId: data.busId, status: 'DRIVER_STARTED_ROUTE' } })
        })
        ...
        const parentIds = (await prisma.student.findMany({
            where: { routeId: data.routeId, isActive: true, parentId: { not: null }, ...(data.busId ? { OR: [{ busId: data.busId }, { busId: null }] } : {}) },
