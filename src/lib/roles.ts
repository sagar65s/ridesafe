...
export function canCreateRole(actorRole: string, targetRole: UserRole): boolean {
  ...
  return actorRole === 'ADMIN' && ['DRIVER', 'PARENT'].includes(targetRole)
}

export const ADMIN_TAB_ACCESS: Record<'SUPER_ADMIN' | 'SCHOOL_ADMIN' | 'ADMIN', readonly string[]> = {
  SUPER_ADMIN: ['ORGANIZATIONS', 'SUPERUSERS', 'OVERVIEW', 'FLEET', 'STUDENTS', 'ATTENDANCE', 'LIVETRIPS', 'SCHEDULE', 'HISTORY', 'MAINTENANCE', 'LOSTFOUND', 'ANNOUNCEMENTS', 'ANALYTICS', 'MESSAGES', 'CALENDAR', 'ISSUES', 'AUDIT', 'SETTINGS'],
  SCHOOL_ADMIN: ['OVERVIEW', 'FLEET', 'STUDENTS', 'ATTENDANCE', 'LIVETRIPS', 'SCHEDULE', 'HISTORY', 'USERS', 'MAINTENANCE', 'LOSTFOUND', 'ANNOUNCEMENTS', 'ANALYTICS', 'MESSAGES', 'ISSUES'],
  ADMIN: ['OVERVIEW', 'FLEET', 'STUDENTS', 'ATTENDANCE', 'LIVETRIPS', 'SCHEDULE', 'HISTORY', 'ANNOUNCEMENTS', 'ISSUES'],
}
